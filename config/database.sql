create type gender_type as enum ('M', 'F')\
 ...
 create table users ( id uuid primary key, first_name varchar(20) not null, last_name varchar(20) not null, gender gender_type not null,password_hash varChar(1024) not null, email varchar(100) unique not null,country varchar(30) not null ) 
 
 create table wallets ( id uuid primary key default gen_random_uuid(), user_id uuid not null, currency char(3) not null, balance numeric(20,6) not null default 0, version int not null default 0, created_at timestamp default now(), constraint user_constraint foreign key (user_id) references users(id) on delete cascade, unique(user_id, currency) ) 
